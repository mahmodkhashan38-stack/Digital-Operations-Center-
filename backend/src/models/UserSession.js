...
const REVOKE_REASONS = [
  ...
  'PASSWORD_RESET', // Manager emergency reset (DOC-57 Flow B) or the
  // self-service, email-delivered temporary password (controllers/
  // auth.controller.js's `forgotPassword`, originally SMS-delivered under
  // Sprint 7 - see git history) - DOC-70's original Manager-approval flow
  // that also used this same reason has been retired (see models/
  // PasswordResetRequest.js), but the reason itself is unchanged: it still
  // always means "this account's password was reset by someone/something
  // other than the account owner's own in-session choice."
